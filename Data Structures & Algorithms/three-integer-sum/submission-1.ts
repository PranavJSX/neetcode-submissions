class Solution {
    ...
    threeSum(nums: number[]): number[][] {
        ...
        for(let i=0; i<sorted_array.length-2; i++){
            ...
            let target = -sorted_array[i];
            while(left<right){
                if(sorted_array[left] + sorted_array[right] === target){
                    ans.push([sorted_array[i], sorted_array[left], sorted_array[right]]);
                    left++;
                    right--;

                    while(left<right && sorted_array[left] === sorted_array[left-1]){
                        left++;
                    }
                    while(left<right && sorted_array[right] === sorted_array[right+1]){
                        right--;
                    }
                }
